...
if not map_osz then
    ...
    for dir in path:gmatch("[^/]+") do
        ...
        -- Получаем имя карты без расширения (любой вариант: .osz или .zip)
        local folder_name = map_osz:match("([^/\\]+)%.([^%.]+)$")
        if not folder_name then
            print("Bad filename")
            os.exit(1)
            end
            folder_name = folder_name:match("(.+)$") -- просто имя без расширения
            ...
            -- Используем %q чтобы корректно экранировать имя файла и директории
            local cmd = string.format("unzip -o %q -d %q", map_osz, target_dir)
            print("Running: " .. cmd)
            os.execute(cmd)
